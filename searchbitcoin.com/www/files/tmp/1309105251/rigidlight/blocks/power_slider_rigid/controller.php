...
class PowerSliderRigidBlockController extends BlockController {
	
	var $pobj;
	
	protected $btTable = 'btPowerSlideRigid';
	...
	function loadImages(){
		...
		$sql = "SELECT * FROM btPowerSlideImgRigid WHERE bID=".intval($this->bID).' ORDER BY '.$sortChoices[$this->playback];
		$db = Loader::db();
	...
	function delete(){
		$db = Loader::db();
		$db->query("DELETE FROM btPowerSlideImgRigid WHERE bID=".intval($this->bID));		
		parent::delete();
	}
	...
	function duplicate($nbID) {
		...
		foreach($this->images as $im) {
			$db->Execute('insert into btPowerSlideImgRigid (bID, fID, pageID, powerSlidePhraseTitle, powerSlidePhraseDesc, groupSet, position) values (?, ?, ?, ?, ?, ?, ?)', 
	...
	function save($data) { 
		...
		if( $data['type'] == 'FILESET' && $data['fsID'] > 0){
			...
			foreach($data['imgFIDs'] as $imgFID){ 
				...
				$db->query("INSERT INTO btPowerSlideImgRigid (bID,fID,pageID,powerSlidePhraseTitle,powerSlidePhraseDesc,groupSet,position) values (?,?,?,?,?,?,?)",$vals);
				$pos++;
			}
		}
