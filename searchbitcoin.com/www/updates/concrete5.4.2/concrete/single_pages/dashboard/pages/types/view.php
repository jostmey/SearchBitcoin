...
if ($ctEditMode) { 
	...
	<tr>
		<td colspan="3" class="subheader" class="subheader"><?php echo t('Default Attributes')?></td>
	</tr>
	<?php 
...
<?php  
	...
	<tr>
		<td colspan="3" class="subheader"><?php echo t('Default Attributes to Display')?></td>
	</tr>
	<?php 
		$attribs = CollectionAttributeKey::getList();
		$i = 0;
	...
	<tr>
		...
		<?php  if ($ct->getMasterCollectionID()) {?>
			...
			if ($tp->canAccessPageDefaults()) { ?>
				<?php  print $ih->button(t('Defaults'), $this->url('/dashboard/pages/types?cID=' . $ct->getMasterCollectionID() . '&task=load_master'))?>
			<?php  } else { 
		...
		<td><?php  print $ih->button(t('Settings'), $this->url('/dashboard/pages/types?ctID=' . $ct->getCollectionTypeID() . '&task=edit'))?></td>
		<?php  if ($cap->canAccessComposer()) { ?>
			<td><?php  print $ih->button(t('Composer'), $this->url('/dashboard/pages/types/composer', 'view', $ct->getCollectionTypeID()))?></td>
		<?php  } ?>	
	</tr>
	<?php  } ?>
