 ...
 * @var integer $new_cutoff defines how many items in the table are considered "new".
 */
...
try
{
	...
	while($cur_time <= $max_time)
	{
		$items=$database->query(
			...
			"AND created < ( SELECT created FROM Items ORDER BY created DESC LIMIT 1 OFFSET $new_cutoff ) ".
			"ORDER BY RAND()"
		);
		foreach($items as $item)
		{
			...
			$database->prepare(
				...
				"( product_id, channel, vendor_id, url, title, description, image, price, time ) ".
				"VALUES ".
				"( :product_id, 'new', :vendor_id, :url, :title, :description, :image, :price, FROM_UNIXTIME( :time ) )"
			)->execute(
				array(
					':product_id'=>$id,
					':vendor_id'=>$id_vendor,
